//+------------------------------------------------------------------+
//| GoldTrendRider_Gold.mq5 - GOLD LEG (attach to an XAUUSD chart)     |
...
#define EA_TAG "[GoldTrendRider_Gold]"
...
input int              InpMagicNumber          = 20260822;

input group "=== Safety ==="
input string           InpExpectedSymbol       = "XAUUSD";  // warns (does not block) if attached to the wrong chart by mistake
...
void TryEnter()
{
   ...
   if(isBuy)
   {
      if(trade.Buy(lots, _Symbol, 0.0, slN, tpN, "GoldTrendRider_Gold-Buy"))
   ...
   {
      if(trade.Sell(lots, _Symbol, 0.0, slN, tpN, "GoldTrendRider_Gold-Sell"))
